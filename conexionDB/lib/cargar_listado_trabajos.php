<?php
	...
	$consulta = "
		SELECT 
			...
			TTP.PUNTUACION AS PUNTUACION_TRABAJO
		FROM TRABAJOS T 
			...
			LEFT JOIN TIPOS_TRABAJO_PUNTUACION TTP ON (TTP.ID_TRABAJO = TT.ID AND T.FECHA BETWEEN TTP.FECHA_DESDE AND TTP.FECHA_HASTA) 
		WHERE
			C.ID = ". $_POST['id'] ." AND
			T.ACTIVO = 1 
		ORDER BY
		...
		if(mysql_num_rows($result) > 0){
			...
			foreach ($rowsC as $row) {
				...
				$tabla .= "<td style='text-align:right'>".$row["PUNTUACION_TRABAJO"]."</td>";
